...
**Visko Orbis Dynamic** is the companion model, with its own template at [`../visko-orbis-dynamic`](../visko-orbis-dynamic). Same Next.js and typed-SDK shape, and the same ancillary surface: a generate-sound toggle on `set_audio_enabled` and a resolution picker rendered from `available_resolutions` (stable offers named delivery tiers; dynamic's list also carries `native`). The one real difference is the prompt pipeline — dynamic has no structured-caption echo message, so stable's caption line has no counterpart there.
